...
def _check_llm_prompts(config_dir: Path, bot_config: BotConfig) -> list[ConfigError]:
    ...
    if bot_config.llm_daily_thought_enabled:
        ...
        for pool_name in ("daily_depth", "daily_style"):
            if not load_prompt_pool(config_dir / "prompts" / pool_name):
                errors.append(
                    ConfigError(
                        file=str(config_dir / "prompts" / pool_name),
                        message=(
                            "llm.dailyThought.enabled is true but no *.txt "
